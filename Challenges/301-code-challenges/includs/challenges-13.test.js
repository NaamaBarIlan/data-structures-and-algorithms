...
const standardizePhoneNumbers = (arr) => {
  const newArr = [];
  for(let i = 0; i < arr.length; i++) {
    const partOne = arr[i].substring(1,4); 
    const partTwo = arr[i].substring(6,9);
    const partThree = arr[i].substring(10,14);
    const newNum = `${partOne}${partTwo}${partThree}`;
    newArr.push(newNum);
  }
  return newArr;
};
